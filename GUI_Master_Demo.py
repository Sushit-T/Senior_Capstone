                ...
                while(approachProcess):
                    ...
                    if success:
                        ...
                        if(curr_data >= globals.APPROACH_COARSE_SETPOINT):
                            ...
                            vpiezo_tip, stepper_motor_steps = self.auto_move_tip(stepper_motor_steps, globals.APPROACH_COARSE_STEP_NM, globals.DIR_DOWN)
                        self.update_label()
                ...
                while(approachProcess):
                    ...
                    if success:
                        ...
                        if(curr_data >= curr_setpoint):
                            ...
                            if adjust_success:
                                stepper_motor_steps -= globals.INC_EIGHT
                                TUNN_APPROACH_ESCAPE_FLG = 1
                                approachProcess = False
                                break
                                #return 1, curr_data, vb_V, vp_V, stepper_motor_steps
                            else:
                            ...
                            vpiezo_tip, stepper_motor_steps = self.auto_move_tip(stepper_motor_steps, globals.APPROACH_STEP_SIZE_NM, globals.DIR_DOWN)
                ...
                while True:
                    ...
                    if success:
                        ...
                        if((curr_data * curr_data) < (globals.CONTROLLER_MIN_CURR * globals.CONTROLLER_MIN_CURR)):
                            vpiezo_tip, stepper_motor_steps = self.auto_move_tip(stepper_motor_steps, globals.CONTROLLER_CONST_STEP_SZ_NM, globals.DIR_DOWN)
                            sum = 0
                        # Use feedback control to maintain target current
                            ...
                            if(controller_output < 0):
                                vpiezo_tip, stepper_motor_steps = self.auto_move_tip(stepper_motor_steps, -controller_output, globals.DIR_UP)               
                            else:
                                vpiezo_tip, stepper_motor_steps = self.auto_move_tip(stepper_motor_steps, controller_output, globals.DIR_DOWN)
...
############################################# CAPACITANCE APPROACH #################################################
    ...
    def convert_curr_to_cap(self):
        """
        Converts the current measurement to a capacitance measurement.
        """
        global cap_data
        global curr_data
...
############################################# END OF ENABLE PERIODIC DATA #################################################
    ...
    def saveKd(self, _=None):
        """
        Method to save and update the Kd variable.
    ...
    def saveKi(self, _=None):
        """
        Method to save and update the Ki variable.
    ...
    def saveSampleRate(self, _=None):
        ...
        if self.check_connection():
            ...
            if self.sample_rate_var.get() == "62.5 kHz":
                sample_rate_save = 62500
            elif self.sample_rate_var.get() == "40 kHz":
                sample_rate_save = 40000
            elif self.sample_rate_var.get() == "20 kHz":
                sample_rate_save = 20000
...
class GraphGUI:
    ...
    def update_graph(self):
        ...
        *Updates every 36ms
        ...
        if CAP_APPR_FLAG:
            self.y_data.append(cap_data)    # Capacitance approach will display the capacitance
        else:
            self.y_data.append(curr_data)   # Otherwise, will display current
    ...
    def reset_graph(self):
        ...
        if CAP_APPR_FLAG:
            self.ax.set_ylabel('Capacitance (fF)')  # femtofarads
        else:
            self.ax.set_ylabel('Current (nA)')\
